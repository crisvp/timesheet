...
def analyze(
    ...
    for row in [x for x in timesheet_log.entries if x[0] <=
        ...
        entry_dur = min(entry_stop, period_stop) - \
            max(entry_start, period_start)
        entry_message = row[2]
        entry_day = util.date2string(
            max(entry_start, period_start)).split(' ')[0]
        entry_day = datetime.strptime(
            entry_day, '%Y/%m/%d').strftime('%a %Y/%m/%d')
...
def main(argv):
    ...
    if command == 'start':
        ...
        if argument == '':
            ...
            period_start = util.get_week_start(
                start_day, start_time, week=util.interpretdate(argument))
